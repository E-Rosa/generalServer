...
var NotesObj = {
    ...
    creationDate: '',
    tags: '',
    createNotes: (parent: HTMLElement, notes: Note[]):void=>{
        notes.forEach((note: Note)=>{
            ...
            let noteTitle = document.createElement('h3') as any;
            let noteText = document.createElement('p') as any;
            ...
            let tagsElement = document.createElement('span') as any;
            ...
            function saveIdWrap(): void{
                saveId(note.note_id);
            }
            
            function updateNoteState(e: any){
                if(e.key === 'Enter'){
                    e.preventDefault();
                    noteContainer.remove();
                    let newNote: Note = {
                        ...note,
                        [e.target.key]: e.target.textContent
                    };
                    notes.pop();
                    notes.push(newNote);
